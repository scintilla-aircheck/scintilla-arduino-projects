...
<form action='/configure' method='POST'>Wifi Details:<br>\
  ...
  Azure Connection String:<input type='text' name='CONNSTR' placeholder='Go get it from Azure!'><br>\
...
int length;
void saveSettingsToEEPROM(String ssid, String password, String cs, String geo) {
  ...
  id = GetStringValue(splitStringByIndex(splitStringByIndex(cs, ';', 1), '=', 1));
...
void doConfigure() {
  ...
  if (server.hasArg("SSID") &&
      ...
      server.hasArg("CONNSTR")) {
    // TODO: Try a connection.
    // TODO: If it's good, save it to EEPROM
    saveSettingsToEEPROM(server.arg("SSID"), server.arg("PASSWORD"), server.arg("CONNSTR"), server.arg("GEO"));
    msg = "Settings saved!\n";
  }
  else {
